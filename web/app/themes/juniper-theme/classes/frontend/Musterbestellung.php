...
add_action( 'init', function () {
	class WC_Product_Musterbestellung extends WC_Product_Simple {
		...
		public function get_shipping_class(): string {
			return parent::get_shipping_class();
		}
	}
} );
...
add_action( 'woocommerce_product_data_panels', function () {
	...
	if ( $product_object
	     && 'musterbestellung' === $product_object->get_type()
	) {
		wc_enqueue_js( "
...
add_action( 'woocommerce_before_add_to_cart_button',
	'add_custom_fields_to_musterbestellung_frontend' );
	...
	if ( 'musterbestellung' === $product->get_type() ) {
		...
		for ( $i = 1; $i <= 3; $i ++ ) {
			...
			woocommerce_form_field( '_associated_product_' . $i, [
				...
				'label'   => sprintf( __( 'Select Associated Product %d',
					'woocommerce' ), $i ),
				'options' => $options,
				'default' => $selectedValue,
				// Use the 'selected' parameter to pre-select the option
			] );
		}
		echo '</div>';
	}
...
function enqueueCustomJsForMusterbestellung() {
	...
	if ( is_product() && $product->get_type() === 'musterbestellung' ) {
		wp_enqueue_script( 'single-musterbestellung-js',
			get_template_directory_uri()
			. '/assets/js/single-musterbestellung.js', [ 'jquery' ], '', TRUE );

		// Localize script to inject PHP values if necessary (demonstrative; not used directly in your JS)
		wp_localize_script( 'single-musterbestellung-js',
			'singleMusterbestellungParams', [
	...
	wp_enqueue_script( 'custom-musterbestellung-js',
		get_template_directory_uri() . '/assets/js/custom-musterbestellung.js',
		[ 'jquery' ], '', TRUE );
	...
	wp_localize_script( 'custom-musterbestellung-js',
		'customMusterbestellungParams', [
			'ajaxurl'                  => admin_url( 'admin-ajax.php' ),
			'restUrl'                  => get_rest_url(),
...
function pullMusterbestellungProductsFromCookie(): array {
	...
	return array_map( function ( $productId ) {
		...
		$local_product->image
			                 = wp_get_attachment_image_src( get_post_thumbnail_id( $productId ),
...
add_action( 'rest_api_init', function () {
	register_rest_route( 'wps/v1', '/musterbestellung/', [
		...
		// Adjust permissions based on your needs
	] );
} );
 ...
 */
function manageSampleboxProductInCart( bool $sampleItemsNeeded ): void {
	...
	$musterboxInCart          = array_filter( $cart, fn( $cartItem ) => wc_get_product( $cartItem['product_id'] ) instanceof WC_Product_Musterbestellung );
...
function cartItemIsSample( mixed $cartItem ): bool {
	return isset( $cartItem['data'] ) && $cartItem['data'] instanceof WC_Product_Musterbestellung;
}
...
function updateMusterbestellungProducts( WP_REST_Request $request ): WP_REST_Response {
	...
	// adjustment not needd as long as product ids ain't getting updates in meta data as well
	//	$updatedProductIds = adjustProductIdsBasedOnCurrentLanguage($updatedProductIds);
	...
	if ( ! $cartNeedsUpdate ) {
		$products = array_map( 'generateProductFrontendElement',
			$updatedProductIds );
	...
	foreach ( $samplesCurrentlyInCart as $cartItemKey => $cartItem ) {
		$productSampleData = array_map( 'setupMusterbestellungData',
			$updatedProductIds );
		WC()->cart->cart_contents[ $cartItemKey ]['musterbestellung_custom_data']
		                   = $productSampleData;
	}
...
function adjustProductIdsBasedOnCurrentLanguage( array $updatedProductIds,
): array {
	return array_map( fn( $productId ) => apply_filters( 'wpml_object_id',
		$productId, 'post', TRUE ), $updatedProductIds );
}
...
function generateProductFrontendElement( string $id ): stdClass {
	...
	$product->image = wp_get_attachment_image_src( get_post_thumbnail_id( $id ),
		'single-post-thumbnail' );
...
add_filter( 'woocommerce_add_cart_item_data',
	'add_musterbestellung_data_to_cart_item', 10, 3 );
function add_musterbestellung_data_to_cart_item(
	$cart_item_data,
	$product_id,
	$variation_id,
) {
	...
	if ( count( $musterbestellungProducts ) > 0 ) {
		$musterbestellungData                           = array_map( 'setupMusterbestellungData',
			$musterbestellungProducts );
...
add_action( 'woocommerce_checkout_create_order_line_item',
	'save_musterbestellung_data_with_order', 10, 4 );

function save_musterbestellung_data_with_order(
	$item,
	$cart_item_key,
	$values,
	$order,
...
add_filter( 'woocommerce_add_to_cart_validation',
	'limit_musterbestellung_product_in_cart', 10, 3 );

function limit_musterbestellung_product_in_cart(
	$passed,
	$product_id,
	$quantity,
) {
...
add_filter( 'woocommerce_quantity_input_args',
	'musterbestellung_quantity_input_args', 10, 2 );
function musterbestellung_quantity_input_args( $args, $product ) {
	if ( ! $product instanceof WC_Product_Musterbestellung ) {
		return $args;
	}

	$args['max_value'] = 1;  // Set maximum quantity to 1
	$args['min_value']
	                   = 0;  // Set minimum quantity to 0 to enforce single item only

	return $args;
}

function enqueue_disable_add_to_cart_script(): void {
	// TODO: please check with the client if normal products should be allowed in cart, if a sample product is already existent
	return;
	...
	if ( $musterbestellung_in_cart ) {
		wp_enqueue_script( 'disable-add-to-cart',
			get_template_directory_uri() . '/assets/js/disable-add-to-cart.js',
			[], FALSE, TRUE );
	}
}
...
function maybe_load_cart() {
	if ( empty( $_SERVER['REQUEST_URI'] ) || ! WC()->is_rest_api_request() ) {
		return;
	}
	...
	$is_my_endpoint = ( str_contains( $req_uri, $rest_prefix ) );
	...
	if ( NULL === WC()->session ) {
		$session_class = apply_filters( 'woocommerce_session_handler',
			'WC_Session_Handler' ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
